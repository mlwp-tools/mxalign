...
class Dates:
    def __init__(self, start: str | np.datetime64, end: str | np.datetime64, period: str | np.timedelta64, range: str | np.timedelta64, step: str | np.timedelta64, ens_size: int = 1, start_from = 1):
    ...
    def substitute(self, path: str):
        ...
        paths = pattern.substitute(
                ...
                {"x": list(range(self.start_from, self.ens_size+1))},
